...
namespace Custom_Mascarello
{
    class Search_Tampa_Inspec
    {
        ...
        public static string SearchMember(string fam, string Inicio_DB_PART_NAME, string[] Expressao, double[] valor_expressao, int Nr_Attr_Concatenar_Nome, double[] Tolerancia, string Cod_DataSul, string descricao)
        {
            ...
            //MessageBox.Show("aaa" + arquivo_substituto"+BBB)
        ...
        {
            string gLog_FileName = @"c:\temp\" + DateTime.Now.ToString("yyyyMMdd") + "Search.log";
        ...
        public static string Localizar_Membro_da_Familia(string Familia, string Inicio_DB_PART_NAME, string[] Atributo, double[] Valor, int Nr_Attr_Concatenar_Nome, double[] Tolerancia, string Cod_DataSul, string descricao)
        {
           // frmPrincipal.Resultado_Listar.Clear();
            ...
            while (flg1)
            {
                ...
                try
                {
                    Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) Abrindo Familia:" + Familia);
                    PartLoadStatus LdSt;
                    Part obj_familia;
                    ...
                    {
                        Registrar_no_LOG(" DEBUG -> (aaaa) " + Atributo[i]);
                    }
                    for (j = 0; j < attr.Length; j++)
                    {
                        ...
                        for (int i = 0; i < Atributo.Length; i++)
                            if (Atributo[i] == atb)
                            {
                                Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) " + Atributo[i] + " Index: " + j.ToString());
                                att_idx[i] = j;
                            }
                    }

                    Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) Procurando por Membros com os Atributos:");
                     ...
                     * como parametro.
                     */
                    Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA)");
                    theUfSession.Part.AskFamilies(obj_familia.Tag, out j, out families);
                    theUfSession.Fam.AskFamilyData(families[0], out family_data);
                    Tag[] attributos = family_data.attributes;


                    Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) attributos.Length:" + attributos.Length.ToString());
                    ...
                    bool flg = true;
                    Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) qtd membros da Familia: " + family_data.member_count);
                    for (int member_idx = 0; member_idx < family_data.member_count; member_idx++)
                    {
                        ...
                        for (j = 0; j < Atributo.Length; j++)
                        {
                            ...
                            Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) Att normal");
                            Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) member_data.values[0]: " + member_data.values[0]);
                            Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) aux_valor_1: " + aux_valor_1);
                            Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) aux_valor_2: " + aux_valor_2);
                            Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) resultado..: " + resultado + " Tol: " + Tolerancia[j]);
                            if (member_idx == 0)
                                Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) Tolerancia.: " + Tolerancia[j]);
                        ...
                        if (Membro_Ja_Existe)
                        {
                            ...
                            Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) Valor Encontrado:");
                            for (int i = 0; i < Atributo.Length; i++)
                            {
                    ...
                    for (int i = 0; i <= Result.Count - 1; i++)
                    {
                        ...
                        membro = Result[i].Substring(0, 6);
                        Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) Item dentro da Tolerância: " + Result[i].Substring(0, 6));
                        Membro_Ja_Existe = true;
                    }
                    if (!Membro_Ja_Existe)
                    {
            ...
            Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia_PESQUISA) Retorno: " + membro);
            return membro;
        }
