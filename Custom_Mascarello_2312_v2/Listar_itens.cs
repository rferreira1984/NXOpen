    ...
    class Listar_itens
    {
        ...
        public static string CreateMember(string fam, string Inicio_DB_PART_NAME, string[] Expressao, double[] valor_expressao, int Nr_Attr_Concatenar_Nome, double[] Tolerancia, string Cod_Atual, string descricao)
        {
            ...
            try
            {
                ...
                arquivo_substituto = Localizar_Membro_da_Familia(encoded_familia);
        ...
        public static string Localizar_Membro_da_Familia(string Familia)
        {
           // bool flg1 = true;
            string membro = "";
            //while (flg1)
            ...
            Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia) Abrindo Familia:" + encoded_familia);
                   ...
                   // int[] att_idx = new int[Atributo.Length];
                    ...
                    theUfSession.Fam.AskFamilyData(families[0], out family_data);
                      ...
                      theUfSession.Fam.AskMemberRowData(families[0], 1, out member_data);
                 Tag[] attr = family_data.attributes;
                    for (j = 0; j < attr.Length; j++)
                    {
                        ...
                        //    if (Atributo[i] == atb)
                            {
                                Registrar_no_LOG(" DEBUG -> (Localizar_Membro_da_Familia) " + attr[j]);
                            //    att_idx[i] = j;
                            }
                    }
